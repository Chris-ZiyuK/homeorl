...
%
% To compile:
%   1. Download NeurIPS 2024/2025 style from https://neurips.cc
%      and place neurips_2024.sty in this directory.
%   2. pdflatex main.tex && bibtex main && pdflatex main.tex && pdflatex main.tex
%
% If you don't have the NeurIPS style file, this will still compile
% with the fallback article class below.
% ============================================================================

% --- Try NeurIPS style; fall back to plain article ---
\newif\ifneurips
...
on new tasks~\cite{Parisi2019,kirkpatrick2017,continualworld2021}. A rich
toolkit of methods has emerged to combat forgetting: weight regularization
(EWC~\cite{kirkpatrick2017}), experience replay~\cite{rolnick2019}, progressive
architectures~\cite{rusu2016}, and policy distillation. These methods are
...
World~\cite{continualworld2021} and CORA~\cite{cora2022}, predominantly
...
Successor features (SF;~\cite{Barreto}) enable transfer by decomposing
...
behavior without retraining. Tomov et al.~\cite{Tomov} show that humans
...
pixel control~\cite{jaderberg2017}, reward prediction, and curiosity-driven
exploration~\cite{pathak2017}. \hace{} differs from these in that the
...
\begin{itemize}[leftmargin=*,topsep=1pt,itemsep=1pt]
  ...
  \item \textbf{Biologically motivated}: mirrors how biological agents
        ...
        states~\cite{keramati2014,yoshida2018,konidaris2006}.
\end{itemize}
...
\textsc{Hazard Gauntlet}& 6 & 14 & 1 & 1 & 4 & -- & Dense hazards \\
...
\textsc{Gauntlet Refuel}& 7 & 16 & 1 & 2 & 4 & -- & All skills combined \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  Error bars: $\pm1\sigma$ across seeds.}
  \label{fig:pilot}
\end{figure}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Publication-quality figures}: Regenerate all plots with
        95\% bootstrap CI, proper labels, consistent colorscheme.
...
\begin{enumerate}[leftmargin=*]
  \item Sham controls: $B_{\text{sham}}$ (uninformative scalar matched
        to energy statistics) and $B_{\text{perm}}$ (temporally broken
        energy observation).
  \item \hace{} coefficient sensitivity: vary $\alpha$ in
        \{0.1, 0.5, 1.0, 2.0\}.
  \item Step cost ablation: $c_{\text{step}} \in \{1, 2, 3\}$.

...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--config", type=str, default="configs/cw_experiment.yaml")
        ...
        for seed_idx in seed_indices:
            ...
            if single_task is not None:
                ...
                for task_i, task_name in enumerate(seq_spec.tasks):
                    env_t = make_cw_env(task_name=str(task_name), seed=seed + task_i * 10_000, hace=True, hace_alpha=alpha, hace_beta=beta)
                    model.set_env(env_t)
                    model.learn(total_timesteps=int(steps_per_task), reset_num_timesteps=False, progress_bar=True)
                    env_t.close()

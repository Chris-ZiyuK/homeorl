...
class _FallbackVarRef:
    """Hashable variable reference with `deref()` for older/newer backends."""

    def __init__(self, v: tf.Variable):
        self._v = v

    def deref(self) -> tf.Variable:
        return self._v
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, _FallbackVarRef) and other._v is self._v


def _var_ref(v: tf.Variable):
    # TF/Keras compatibility:
    # - `tf.Variable.experimental_ref()` exists in newer TF and returns an object with `.deref()`.
    # - Some installs expose `.ref()` (callable) instead.
    if hasattr(v, "experimental_ref") and callable(getattr(v, "experimental_ref")):
        return v.experimental_ref()
    if hasattr(v, "ref") and callable(getattr(v, "ref")):
        return v.ref()
    return _FallbackVarRef(v)
...
class PackNet_SAC(SAC):
    def __init__(
        ...
        for model in packnet_models:
            ...
            for v in variables_to_manage:
                ref = _var_ref(v)
                self.managed_variable_refs.add(ref)
                if "kernel" in v.name:
                    self.owner[ref] = tf.Variable(
                        tf.zeros_like(v, dtype=tf.int32), trainable=False
                    )
                    self.saved_variables[ref] = tf.Variable(tf.zeros_like(v), trainable=False)
    ...
    def on_task_end(self, current_task_idx: int) -> None:
        ...
        if current_task_idx < self.env.num_tasks - 1:
            ...
            # SAC uses separate optimizers under Keras 3.
            ...
            if getattr(self, "alpha_optimizer", None) is not None:
    ...
    def _adjust_gradients_list(
        ...
        for g, v in zip(grads, variables):
            ref = _var_ref(v)
            if ref in self.managed_variable_refs:
                if "kernel" in v.name:
                    res.append(g * tf.cast(self.owner[ref] == seq_idx, tf.float32))
                else:
                    res.append(
